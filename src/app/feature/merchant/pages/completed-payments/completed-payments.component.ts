...
import { IMandateSchedules } from '../../models/mandate-schedules.model';
...
export class CompletedPaymentsComponent implements OnInit {
  ...
  pageLabel = 'Completed Payment List';
  completedMandatePaymentList: any[] = [];
  options = {};
  data1 = [];
  dataBK = [];
  columns: any = {};
  ...
  ngOnInit(): void {
    this.isLoading = true;
    this.loadDatatable();
    this.loadcompletedMandatePayments();
  }
  ...
  loadcompletedMandatePayments(){
    this.mandateService.getCompletedPayments().subscribe({
      next: (res: any) => {
      //  console.log(res);
        this.completedMandatePaymentList = res.result;
